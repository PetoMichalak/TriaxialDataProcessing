...
path = "/home/pet5o/workspace/TDP/data/150426_1136_workflowTests/testingSets/synced/annotated/stream_split"
...
extractFeatures = function(item) {
  ...
  # save to the disk
  write.csv(output, 
            paste(file_path_sans_ext(item),"_features.csv",sep=""), 
            row.names=FALSE)
}
...
require(parallel)
cl <- makeCluster(4)
clusterExport(cl, c("SPLIT_INTERVAL", "FREQUENCY", "SET_BOUNDARY", 
                    "TOP_FREQ", "getColNames", "extractFFT", 
                    "fft.profile", "file_path_sans_ext"))
parLapply(cl, filenames, extractFeatures)
stopCluster(cl)